function [I,Hr_s,Hr] = fMIfromDensities(Fcond,q)
%FMIFROMDENSITIES Calculates the mutual information from given
%signal-response conditional probability density matrix
%
...
Wq = cellfun(@(q,f) ones(size(f))*q/length(ones(size(f))),num2cell(q),F,'uniformoutput',0); 
Wq = cat(1,Wq{:}); 
Fall = cat(1,F{:});
Fall(Fall<=eps)=nan; 
